...
The most commonly observed snowfall value is `ny_noaa_clean %>% group_by(snow) %>% count(snow) %>% summarize(common_obs_snow = max(n, na.rm = TRUE)) %>% arrange(desc(common_obs_snow)) %>% top_n(1) %>% select(-common_obs_snow)`. Since it does not snow all year around in NY it makes sense that there are a lot of zero values. The most common nonzero snowfall is `ny_noaa_clean %>% group_by(snow) %>% count(snow) %>% summarize(common_obs_snow = max(n, na.rm = TRUE)) %>% arrange(desc(common_obs_snow)) %>% top_n(3) %>% select(-common_obs_snow)`. 
...
ny_noaa_jan_july = ny_noaa_clean %>% 
  ...
  summarize(avg_tmax_year_month = mean(tmax,na.rm = TRUE)) %>% 
  ggplot(ny_noaa_jan_july(aes(x = id, y = avg_tmax_year_month))) + 
