...
The latest average order time for both Coffe Ice Cream and Pink Lady
...
``` r
n_missing_tmax = ny_noaa %>%
  mutate(tmax_na = is.na(tmax)) %>%
  count(tmax_na) %>% filter(tmax_na == TRUE)

n_missing_snow = ny_noaa %>%
  mutate(snow_na = is.na(snow)) %>%
  count(snow_na) %>% filter(snow_na == TRUE)
```

This dataset has 2595176 observations and 7 variables. This dataset
contains readings from 747. There are **134 aisles** and **21
departments** selling a total of **39123 unique products**
